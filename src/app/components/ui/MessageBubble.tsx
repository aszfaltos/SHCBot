    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.userReactions && data.userReactions.length > 0) {
          const hasLike = data.userReactions.some(
            (r: any) => r.type === "like"
          );
          const hasDislike = data.userReactions.some(
  ...
  const handleReaction = async (reaction: "like" | "dislike") => {
    ...
    if (isCurrentlyActive) {
      ...
      if (userReactions[oppositeReaction]) {
        newCounts[oppositeReaction] = Math.max(
          0,
          newCounts[oppositeReaction] - 1
        );
      }
    }
    ...
    try {
      if (isCurrentlyActive) {
        ...
        if (reactionId) {
          ...
          console.warn(
            "ReactionId missing, trying to delete by messageId instead"
          );
          const response = await fetch(`/api/message?messageId=${message.id}`, {
            method: "DELETE",
          });
        ...
        const oppositeReaction = reaction === "like" ? "dislike" : "like";
        if (userReactions[oppositeReaction]) {
          const oppositeReactionId =
            userReactions.reactionIds[oppositeReaction];
